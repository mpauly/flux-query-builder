import { flux, fluxExpression, fluxInteger, FluxParameterLike, fluxString, FLUX_VALUE } from "@influxdata/influxdb-client";
import { FluxFunction } from "../base/function";
import { QueryLine } from "../types/base";
import { ADDITION_KEY, DIVISION_KEY, FluxMapFieldOptions, FluxMapQuery, MULTIPLICATION_KEY, SUBTRACTION_KEY } from "../types/map";
...
export class MapFragment extends QueryFragment {
    ...
    protected collectArgs(): [string, FluxParameterLike | undefined][] {
        if (this.fn instanceof FluxFunction) {
            return [
                ['fn', this.fn.renderFlux()]
            ]
        }
        return [
            ['fn', this.constructMapFunction(this.fn).renderFlux()]
        ]
    }

    protected constructMapFunction(opts: FluxMapQuery) {
        const transformString = Object.entries(opts).map(([key, value]) => this.buildFieldTransform(key, value)).join(`, `)
